...
def snapshot(document: Document) -> dict:
    ...
    for a in document.annotations:
        if a.kind == "label":
            ...
            continue
        obj = objects.setdefault(a.object_id, {"key": a.object_id, "label": a.label, "note": a.note})
        obj[a.kind] = ([*a.points[0], *a.points[1]] if a.kind == "box" else a.points)
    return {"image_sha256": document.image.sha256, "base_revision": revision(document),
            "classifications": classifications, "objects": list(objects.values())}
...
def check_packet(packet: Packet, document: Document) -> None:
    ...
    for view in packet.views:
        ...
        if not (0 <= x1 < x2 <= info.width and 0 <= y1 < y2 <= info.height and 0 <= a < c and 0 <= b < d):
...
def batch_document(batch: Batch, document: Document, packet: Packet | None = None) -> Document:
    ...
    if batch.base_revision != revision(document):
        raise ValueError("stale base_revision; run prepare again and reconcile with current annotations")
    if packet:
        check_packet(packet, document)
    by_object_kind = {(a.object_id, a.kind): a.id for a in document.annotations if a.kind != "label"}
    ...
    annotations = [Annotation(id=by_class.get(c.label) or new_id(c.label, "label"), kind="label",
                              label=c.label, note=c.note) for c in batch.classifications]
    for obj in batch.objects:
        if packet and packet.rules.categories is not None and obj.label not in packet.rules.categories:
            raise ValueError(f"object {obj.key}: label {obj.label!r} is not in the allowed categories")
        ...
        if obj.view_id is not None:
            ...
            def source_point(p: tuple[float, float]) -> tuple[float, float]:
                ...
                if not (a <= x <= c and b <= y <= d):
                    raise ValueError(f"object {obj.key}: point lies outside view content (rulers/padding are not image pixels)")
        ...
        for kind, points in [("box", [box[:2], box[2:]] if box is not None else None), ("polygon", polygon)]:
            if points is not None:
                annotations.append(Annotation(id=by_object_kind.get((obj.key, kind)) or new_id(obj.key, kind),
                                              object_id=obj.key, kind=kind, label=obj.label,
                                              points=points, note=obj.note))
