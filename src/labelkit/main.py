...
def _parser() -> argparse.ArgumentParser:
    ...
    submit = commands.add_parser("submit", help="Submit classifications and objects against a saved task handle")
    submit.add_argument("task", help="task.json returned by task, submit, or task-status")
    submit.add_argument("--file", required=True, help="JSON with classifications and objects only, or - for stdin")
    status = commands.add_parser("task-status", help="Recover the latest task handle and view after lost output")
    status.add_argument("task")
    prepare = commands.add_parser("prepare", help="Create agent views, task rules, schema, and an editable snapshot")
    prepare.add_argument("image")
    prepare.add_argument("--output", "-o", required=True, help="New directory for images and task packet")
    prepare.add_argument("--instructions", help="Researcher instructions in a text file")
    prepare.add_argument("--categories", help="JSON array of allowed object labels (scene classes are separate)")
    prepare.add_argument("--geometry", choices=["boxes", "polygons", "both"], default="both")
    apply = commands.add_parser("apply", help="Atomically replace a complete annotation snapshot and create review views")
    apply.add_argument("image")
    apply.add_argument("--file", required=True, help="Complete batch JSON, or - for stdin; omitted objects are removed")
    apply.add_argument("--packet", help="Source-bound task packet; enforces categories/geometry and resolves view_id")
    apply.add_argument("--output", "-o", help="New review directory; defaults beside the batch file")
    review = commands.add_parser("review", help="Create padded original/annotated crop pages with original-coordinate rulers")
    ...
    review.add_argument("--regions", help="JSON array of {key, box:[x1,y1,x2,y2]} to inspect, even before labeling")
    review.add_argument("--per-page", type=int, default=2, help="Objects per paired review page (1..4)")
    review.add_argument("--padding", type=float, default=.25, help="Surrounding context fraction (>0..2); minimum 16 pixels")
    ...
    export.add_argument("--categories", help="Ordered JSON label list; reuse across exports for stable IDs")
    for name, description in [
        ...
        if name in {"label", "box", "polygon"}:
            ...
            command.add_argument("--id", help="Replace this existing annotation instead of adding one")
            command.add_argument("--note", help="Optional rationale or uncertainty note")
        if name in {"box", "polygon"}:
            command.add_argument("--object-id", help="Attach to an existing object ID returned by a shape command")
        if name == "link":
            command.add_argument("--ids", nargs="+", required=True, help="Annotation IDs to link; first object ID is retained")
        if name == "box":
            command.add_argument("--xyxy", nargs=4, type=float, required=True,
                                 metavar=("X1", "Y1", "X2", "Y2"))
        if name == "polygon":
        ...
        if name == "render":
            command.add_argument("--grid", type=int, default=0, metavar="PIXELS",
    ...
    try:
        if args.command in {"task", "submit", "task-status"}:
            if args.command == "task":
                result = start_task(args.image, args.output, instructions=args.instructions,
                                    categories=args.categories, geometry=args.geometry, max_passes=args.max_passes)
            elif args.command == "submit":
                payload = sys.stdin.read() if args.file == "-" else Path(args.file).expanduser().read_text()
        ...
        if args.command in {"prepare", "apply", "review"}:
            ...
            if args.command == "prepare":
                result = workflow.prepare(args.output, instructions=args.instructions,
                                          categories=args.categories, geometry=args.geometry)
            elif args.command == "apply":
                payload = sys.stdin.read() if args.file == "-" else Path(args.file).expanduser().read_text()
                result = workflow.apply(payload, output=args.output or workflow.default_review_output(args.file),
                                        packet_path=args.packet)
            else:
                result = workflow.review(args.output, packet_path=args.packet, regions=args.regions,
                                         per_page=args.per_page, padding=args.padding)
        ...
        if args.command == "info":
            ...
            result = kit.annotate(args.command, args.label, points, args.id, args.note, getattr(args, "object_id", None))
            ...
            result = kit.export_image(args.output, grid=grid,
        ...
        if args.command in {"task", "submit", "task-status"}:
            result["code"] = error.code if isinstance(error, TaskError) else (
                "VALIDATION_ERROR" if isinstance(error, ValueError) else "IO_ERROR")
            result["recovery"] = error.recovery if isinstance(error, TaskError) else (
            ...
            if isinstance(error, ValidationError):
                ...
                result["details"] = error.errors(include_url=False, include_context=False, include_input=False)[:3]
        if args.command in {"prepare", "apply", "review"}:
            result["code"] = "VALIDATION_ERROR" if isinstance(error, ValueError) else "IO_ERROR"
            if isinstance(error, ValidationError):
                result["details"] = error.errors(include_url=False, include_context=False, include_input=False)
