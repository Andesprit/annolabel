...
class Workflow(AnnoLabel):
    ...
    def prepare(
        ...
        *,
        ...
        rules = Rules(
            geometry=geometry,
            **(
                {"instructions": Path(instructions).expanduser().read_text()}
                if instructions
                else {}
            ),
            categories=json.loads(Path(categories).expanduser().read_text())
            if categories
            else None,
        )
        return write_bundle(
            self.image, self.document, output, rules=rules, prepare=True, short=short
        )
    ...
    def review(
        ...
        *,
        ...
        selected = (
            TypeAdapter(list[Region]).validate_json(Path(regions).expanduser().read_text())
            if regions
            else None
        )
        return write_bundle(
            ...
            self.document,
            output,
            rules=packet.rules if packet else Rules(),
            regions=selected,
            per_page=per_page,
            padding=padding,
        )
    ...
    def apply(
        ...
        *,
        ...
        try:
            ...
            if revision(
                AnnoLabel(str(self.path), image_service=self.image_service).document
            ) != revision(self.document):
        ...
        return {
            **bundle,
            ...
            "objects": [
                {"key": a.object_id, "annotation_id": a.id, "kind": a.kind}
