...
def export_coco(items: list[tuple[Path, Document]], output: str,
    ...
    if (not isinstance(categories, list) or
        any(not isinstance(name, str) or not name.strip() for name in categories) or
    ...
    try:
        ...
        for image_id, (source, doc) in enumerate(items, 1):
            ...
            provenance["images"].append({"image_id": image_id, "source": str(source),
                                         "sha256": info.sha256, "exported_file": relative_image})
            objects = {}
            for annotation in doc.annotations:
                if annotation.kind == "label":
                    classifications.append([image_id, relative_image, annotation.label, annotation.note or ""])
                else:
            ...
            for object_id, members in objects.items():
                ...
                record = {"id": annotation_id, "image_id": image_id,
                ...
                if polygon:
                    ...
                    record["area"] = abs(sum(a[0]*b[1] - b[0]*a[1]
                                             for a, b in zip(p, p[1:] + p[:1]))) / 2
                coco["annotations"].append(record)
                provenance["objects"].append({"annotation_id": annotation_id, "image_id": image_id,
                                              "object_id": object_id,
                                              "source_annotations": [a.model_dump(mode="json") for a in members]})
        annotation_file = "annotations/instances_default.json"
        (temporary / annotation_file).write_text(json.dumps(coco, indent=2, ensure_ascii=False, allow_nan=False) + "\n")
        (temporary / "provenance.json").write_text(json.dumps(provenance, indent=2, ensure_ascii=False) + "\n")
        (temporary / "categories.json").write_text(json.dumps(categories, indent=2, ensure_ascii=False) + "\n")
    ...
    return {"format": "coco", "output": str(destination),
            "annotations": str(destination / annotation_file),
            "images": len(coco["images"]), "objects": len(coco["annotations"]),
            "categories": len(categories), "classifications": len(classifications)}
