...
def _ruler(image: Image.Image, rect: tuple, title: str, grid: bool = False) -> Image.Image:
    ...
    for axis in (0, 1):
        ...
        ticks = (
            [low]
            + list(range(math.ceil(low / step) * step, math.floor(high / step) * step + 1, step))
            + [high]
        )
        positions = []
        for value in sorted(set(ticks)):
            ...
            if axis == 0:
                ...
                draw.text(
                    (max(MARGIN_X, min(x - 10, canvas.width - 40)), 27),
                    label,
                    font=_font(12),
                    fill="white",
                )
...
def write_bundle(
    ...
    *,
    ...
    for item in [] if short else (regions if regions is not None else objects):
        ...
        crop = (
            max(0, math.floor(x1 - px)),
            max(0, math.floor(y1 - py)),
            min(image.width, math.ceil(x2 + px)),
            min(image.height, math.ceil(y2 + py)),
        )
        selected.append((key, label, crop))
    destination.parent.mkdir(parents=True, exist_ok=True)
    temporary = Path(tempfile.mkdtemp(prefix=".annolabel-views-", dir=destination.parent))
    packet = Packet(
        image_sha256=document.image.sha256, width=image.width, height=image.height, rules=rules
    )
    pages = []
    try:
        ...
        packet.views.append(
            View(
                id="original",
                path=str(destination / "original.png"),
                source_rect=rect,
                content_rect=rect,
            )
        )
        if short:
            ...
            packet.views.append(
                View(
                    id="view",
                    path=str(destination / "view.png"),
                    source_rect=rect,
                    content_rect=rect,
                )
            )
        if prepare:
            _ruler(image, rect, "Original oriented pixels: x right, y down", grid=True).save(
                temporary / "grid.png"
            )
            packet.views.append(
                View(
                    id="grid",
                    path=str(destination / "grid.png"),
                    source_rect=rect,
                    content_rect=(
                        MARGIN_X,
                        MARGIN_Y,
                        MARGIN_X + image.width,
                        MARGIN_Y + image.height,
                    ),
                )
            )
        overview = _ruler(
            _outlined(image, document),
            rect,
            "Overview: thin boxes and polygons; coordinates are ORIGINAL pixels",
        )
        legend = [f"{i + 1}. {obj['key']}: {obj['label']}" for i, obj in enumerate(objects)]
        legend_lines = [
            (i, line)
            for i, label in enumerate(legend)
            for line in textwrap.wrap(label, max(30, overview.width // 9))
        ]
        page = Image.new(
            "RGB",
            (overview.width, overview.height + max(30, len(legend_lines) * 22 + 12)),
            "#101827",
        )
        ...
        for line_no, (obj_no, line) in enumerate(legend_lines):
            draw.text(
                (10, overview.height + 6 + line_no * 22),
                line,
                font=_font(),
                fill=COLORS[obj_no % len(COLORS)],
            )
        page.save(temporary / "overview.png")
        packet.views.append(
            View(
                id="overview",
                path=str(destination / "overview.png"),
                source_rect=rect,
        ...
        for start in range(0, len(selected), per_page):
            ...
            for index, (key, label, crop) in enumerate(selected[start : start + per_page], start):
                ...
                annotated = _outlined(image, document, {key} if regions is None else None).crop(
                    crop
                )
                title_lines = textwrap.wrap(
                    f"{index + 1}. {key}: {label} | ruler labels are ORIGINAL coordinates", 90
                )
                heading = 12 + 20 * len(title_lines)
                pair = Image.new(
                    "RGB", (2 * (384 + MARGIN_X) + 12, size[1] + MARGIN_Y + heading), "#101827"
                )
                draw = ImageDraw.Draw(pair)
                for line_no, line in enumerate(title_lines):
                    draw.text((8, 4 + 20 * line_no), line, font=_font(14), fill="white")
                for side, (name, content) in enumerate(
                    ...
                    page_views.append(
                        View(
                            id=f"region-{index + 1}-{name}",
                            path=str(destination / filename),
                            source_rect=crop,
                            content_rect=(
                                x + MARGIN_X,
                                offset_y + heading + MARGIN_Y,
                                x + MARGIN_X + size[0],
                                offset_y + heading + MARGIN_Y + size[1],
                            ),
                        )
                    )
                rows.append(pair)
        ...
        (temporary / "schema.json").write_text(
            json.dumps(Batch.model_json_schema(), indent=2) + "\n"
        )
        guide = (
    ...
    return {
        ...
        "packet": str(destination / "packet.json"),
        "original": str(destination / "original.png"),
        "overview": str(destination / "overview.png"),
        "grid": str(destination / "grid.png") if prepare else None,
        "review_pages": pages,
        "annotations_file": str(destination / "annotations.json"),
        "schema": str(destination / "schema.json"),
        "guide": str(destination / "guide.txt"),
        "revision": snapshot(document)["base_revision"],
