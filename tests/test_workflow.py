...
def test_batch_update_is_atomic_preserves_ids_and_exports(source: Path, tmp_path: Path) -> None:
    ...
    corrected["objects"][0].update(box=[20,20,60,60], polygon=[[20,20],[60,20],[60,60],[20,60]])
    corrected["objects"][0]["label"] = "square"
    result = cli("apply", source, "--file", save(tmp_path/"corrected.json", corrected),
                 "--packet", first["packet"], "-o", tmp_path/"review2")
    assert {a["id"] for a in cli("info", source)["annotations"]} == old_ids
    assert {a["label"] for a in cli("info", source)["annotations"] if a["object_id"] == "obj1"} == {"square"}
    before = Path(result["sidecar"]).read_bytes()
    assert "stale" in cli("apply", source, "--file", payload, "-o", tmp_path/"retry", success=False)["error"]
...
@pytest.mark.parametrize("case", ["outside", "self_intersect", "duplicate", "unknown_view", "extra", "wrong_source"])
def test_invalid_batch_preserves_entire_existing_snapshot(source: Path, tmp_path: Path, case: str) -> None:
    old = cli("box", source, "--label", "keep", "--xyxy", 5,5,10,10)
    context, batch = prepared(source, tmp_path)
    batch["objects"] = [{"key":"new", "label":"leaf", "polygon":[[20,20],[50,20],[35,50]]}]
    if case == "outside": batch["objects"][0]["polygon"][0] = [-1,20]
    if case == "self_intersect": batch["objects"][0]["polygon"] = [[0,0],[90,70],[0,70],[70,0]]
    if case == "duplicate": batch["objects"].append(batch["objects"][0].copy())
    if case == "unknown_view": batch["objects"][0]["view_id"] = "missing"
    if case == "extra": batch["objects"][0]["confidence"] = .9
    if case == "wrong_source": batch["image_sha256"] = "wrong"
    path = Path(old["sidecar"])
    before = path.read_bytes()
    failure = cli("apply", source, "--file", save(tmp_path/"bad.json", batch), "--packet", context["packet"],
                  "-o", tmp_path/"bad-review", success=False)
...
def test_task_rules_geometry_and_category_enforcement(source: Path, tmp_path: Path) -> None:
    ...
    context = cli("prepare", source, "-o", tmp_path/"prep", "--instructions", instructions, "--categories", categories)
    ...
    payload = save(tmp_path/"batch.json", batch)
    assert "allowed categories" in cli("apply", source, "--file", payload, "--packet", context["packet"], success=False)["error"]
    batch["objects"][0]["label"] = "leaf"
    save(payload, batch)
    assert "requires a polygon" in cli("apply", source, "--file", payload, "--packet", context["packet"], success=False)["error"]
...
def test_view_pixels_map_back_with_padding_and_page_offsets(source: Path, tmp_path: Path) -> None:
    region_file = save(tmp_path/"regions.json", [{"key":f"r{i}","box":[20,20,40,40]} for i in range(3)])
    ...
    batch["objects"] = [{"key":"region", "label":"area", "view_id":view["id"],
                         "polygon":[[a,b],[c,b],[c,d],[a,d]]}]
    result = cli("apply", source, "--file", save(tmp_path/"batch.json", batch),
                 "--packet", review["packet"], "-o", tmp_path/"applied")
...
def test_coordinate_context_rejects_padding_and_wrong_image(source: Path, tmp_path: Path) -> None:
    context,batch = prepared(source,tmp_path)
    batch["objects"] = [{"key":"one","label":"shape","view_id":"grid","polygon":[[0,0],[70,50],[80,70]]}]
    payload = save(tmp_path/"batch.json", batch)
    assert "outside view content" in cli("apply", source, "--file", payload, "--packet", context["packet"], success=False)["error"]
    packet = json.loads(Path(context["packet"]).read_text())
    packet["width"] += 1
    assert "does not match" in cli("review", source, "--packet", save(tmp_path/"wrong.json", packet),
                                  "-o", tmp_path/"wrong-review", success=False)["error"]
...
def test_prepare_orientation_and_review_preserves_original_pixels(tmp_path: Path) -> None:
    ...
    with Image.open(context["original"]) as image: assert image.size == (40,80)
    batch["objects"] = [{"key":"one","label":"red area","polygon":[[10,20],[30,20],[30,60],[10,60]]}]
    result = cli("apply", source, "--file", save(tmp_path/"batch.json",batch), "-o",tmp_path/"review")
    ...
    with Image.open(view["path"]) as image, Image.open(context["original"]) as original:
        ...
        expected=original.crop(tuple(view["source_rect"])).resize(crop.size,Image.Resampling.NEAREST)
