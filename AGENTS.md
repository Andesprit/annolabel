# Using AnnoLabel to annotate images

AnnoLabel supplies annotation operations. You, the vision-capable agent, supply the interpretation and geometry. Do not invoke inference models or external labeling services for this workflow.
...
1. Run `uv run --project <ANNOLABEL_DIR> annolabel prepare <IMAGE> --output <NEW_PREP_DIR>`. Include `--instructions <RESEARCHER_TEXT_FILE>`, `--categories <OBJECT_LABELS_JSON>`, and `--geometry boxes|polygons|both` when supplied. The default is both; category restrictions concern object labels, not scene classifications.
...
3. Edit the annotation JSON as a **complete snapshot**, retaining `image_sha256` and `base_revision`. Provide `classifications: [{"label":"scene","note":null}]` and `objects: [{"key":"obj1","label":"object","box":[x1,y1,x2,y2],"polygon":[[x,y],...],"note":null}]`. Choose stable unique object keys; AnnoLabel manages annotation IDs and links each pair automatically. Omitted objects/classes are removed. For segmentation, a polygon alone automatically gets its enclosing box. Do not equate equal labels with equal object identity.
4. Run `annolabel apply <IMAGE> --file <BATCH_JSON> --packet <PREP_DIR/packet.json> --output <NEW_REVIEW_DIR>`. All objects are validated and replaced together; no expand/update/shrink sequences. Output directories must be new. Failed edits preserve the sidecar. If the revision is stale, prepare the current state again and reconcile; do not blindly overwrite another edit.
...
1. Run `uv run --project <ANNOLABEL_DIR> annolabel info <ABSOLUTE_IMAGE_PATH>`. Read the original oriented width/height and existing annotations.
...
```sh
uv run --project <ANNOLABEL_DIR> annolabel box <IMAGE> --label car --xyxy 50 30 250 180
uv run --project <ANNOLABEL_DIR> annolabel polygon <IMAGE> --label leaf --points "[[25,40],[60,20],[90,45],[60,90]]"
uv run --project <ANNOLABEL_DIR> annolabel render <IMAGE> --output <PREVIEW.png>
```

Use `annolabel <COMMAND> --help` for full arguments. Inspect each image before choosing coordinates; example coordinates are not annotations for your image.

## Default training dataset export

Run `uv run --project <ANNOLABEL_DIR> annolabel export <IMAGE_OR_SOURCE_FOLDER> --output <NEW_DATASET_DIR>`. COCO is the default; no format flag is needed. Folder export discovers annotation sidecars recursively and excludes unannotated images without sidecars.
